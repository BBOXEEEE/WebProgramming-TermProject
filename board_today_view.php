...
<body>
    ...
    <?php
	    ...
	    $sql = "SELECT * FROM board_today WHERE num = $num";
            ...
            <div class="item">
                <div class="post-box">
                    ...
                    <div class="content-box">
                        <label><img src="<?=$image_file?>" alt=""><?=$content?></label>
                    </div>
                    <div class="buttons">
                        <button type="button" class="submit" onclick="location.href='./board_today.php?page=<?=$page?>'">목록보기</button>
                        <?php
                            if($userid == $id || $userid == "admin"){
                        ?>
                            <button type="button" class="submit" onclick="location.href=`./board_today_modify.php?num=<?=$num?>&page=<?=$page?>`">수정</button>
                ...
                <div class="comment-box">
                    <div class="input-box">
                        ...
                        <?php
                            if($userid){
                        ?>
                            <form name="comment" method="post" action="./php/comment_today_create.php?num=<?=$num?>&page=<?=$page?>">
                    ...
                    <div id="commentList">
                        <?php
                            $sql = "SELECT * FROM comment_today WHERE post_num = '$num'";
                            $result = mysqli_query($con, $sql);
                                        ...
                                        <div class="comment" style="border-bottom: none;">
                                            ...
                                            <span class="information">
                                                ...
                                                <?php
                                                    if($userid == $id || $userid == "admin"){
                                                ?>
                                                    <a href="./php/comment_today_delete.php?num=<?=$num?>&page=<?=$page?>&comment_num=<?=$comment_num?>">삭제</a>
                                                <?php
                                                }
                                                ?>
                                            </span>
    ...
    <script>
        function checkDelete(){
            ...
            if(result){
                window.location.href = `./php/board_today_delete.php?num=<?=$num?>&page=<?=$page?>`;
            }
            else{
                window.close();
            }
