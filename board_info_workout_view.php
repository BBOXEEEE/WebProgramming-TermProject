...
<body>
    ...
    <?php
	    ...
	    $sql = "SELECT * FROM board_workout WHERE num = $num";
        ...
        if($userid && !(isset($_COOKIE[$hit_cookie_user]))) {                  
            $new_hit = $hit + 1;                                  
            $sql = "UPDATE board_workout SET hit=$new_hit WHERE num=$num";  
        ...
        setcookie("$hit_cookie_user", true, time() + (60*60*24));
        setcookie("$hit_cookie_nouser",true, time() + (60*60*24));
        // mysqli_query($con, $sql);
    ?>
        <section id="grid-bottom">
            <div class="item">
                <div class="post-box">
                    ...
                    <div class="buttons">
                        <button type="button" class="submit" onclick="location.href='./board_info_workout.php?page=<?=$page?>'">목록보기</button>
                        <?php
                            if($userid == $id  || $userid == "admin"){
                        ?>
                            <button type="button" class="submit" onclick="location.href=`./board_info_workout_modify.php?num=<?=$num?>&page=<?=$page?>`">수정</button>
                ...
                <div class="comment-box">
                    <div class="input-box">
                        ...
                        <?php
                            if($userid){
                        ?>
                            <form name="comment" method="post" action="./php/comment_workout_create.php?num=<?=$num?>&page=<?=$page?>">
                    ...
                    <div id="commentList">
                        <?php
                            $sql = "SELECT * FROM comment_workout WHERE post_num = '$num'";
                            $result = mysqli_query($con, $sql);
                                        ...
                                        <div class="comment" style="border-bottom: none;">
                                            ...
                                            <span class="information">
                                                ...
                                                <?php
                                                    if($userid == $id || $userid == "admin"){
                                                ?>
                                                    <a href="./php/comment_workout_delete.php?num=<?=$num?>&page=<?=$page?>&comment_num=<?=$comment_num?>">삭제</a>
                                                <?php
                                                }
                                                ?>
                                            </span>
    ...
    <script>
        function checkDelete(){
            ...
            if(result){
                window.location.href = `./php/board_info_workout_delete.php?num=<?=$num?>&page=<?=$page?>`;
            }
            else{
                window.close();
            }
